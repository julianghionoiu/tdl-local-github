...
def run(command):
    ...
    if command == "console":
        ...
        run_python(python_file, STORAGE_FOLDER, pid_file, True)


def run_python(python_path, storage_dir, pid_file, consoleMode):
    if consoleMode:
        proc = subprocess.call(["python", python_path, storage_dir], cwd=SCRIPT_FOLDER)
    else:
        proc = subprocess.Popen(["python", python_path, storage_dir, "&",], cwd=SCRIPT_FOLDER)
